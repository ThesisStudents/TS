...
## Data Split
X_tr_m1, X_ts_m1, y_tr_m1, y_ts_m1 = train_test_split(df.drop(columns = 'ALERT_STATUS'), 
                                                      y_m1,
                                                      stratify = y_m1,
                                                      test_size=0.33,
                                                      random_state=123)
...
X_tr_pp_m1 = pipe.fit_transform(X_tr_m1)
...
X_res, y_res = ada.fit_resample(X_tr_pp_m1, y_tr_m1) 
...
feat_selector = BorutaPy(boruta_rf, verbose=2, random_state=123,
                         alpha=0.05, perc = 100, max_iter = 16, two_step = False) #Normaal 50 max_iter

feat_selector.fit(X_res.toarray(), y_res)

X_tr_res_bor = feat_selector.transform(X_res.toarray())
X_ts_res_bor = feat_selector.transform(X_ts_pp_m1)
...
print(accuracy_score(y_ts_m1, dt.predict(X_ts_res_bor)))
print(recall_score(y_ts_m1, dt.predict(X_ts_res_bor), average='macro'))
target_names = ['Unsuspicious','Linked2Case', 'Suspicious']
print(classification_report(y_ts_m1, dt.predict(X_ts_res_bor), target_names=target_names))

#Find the optimal hyperparamaters
dt = DecisionTreeClassifier(random_state=123)
param_grid = {'max_depth': list(np.linspace(1, 67, 41, dtype = int)) + [None],
...
print(accuracy_score(y_ts_m1, rf.predict(X_ts_res_bor)))
print(recall_score(y_ts_m1, rf.predict(X_ts_res_bor), average='macro'))
target_names = ['Unsuspicious','Linked2Case', 'Suspicious']
print(classification_report(y_ts_m1, rf.predict(X_ts_res_bor), target_names=target_names))

#Find the optimal hyperparameters
rf = RandomForestClassifier(random_state=123)
param_grid = {'n_estimators': list(np.linspace(1, 200, 100, dtype = int)),
              'max_depth': list(np.linspace(1, 67, 30, dtype = int)) + [None],
              'criterion': ['entropy', 'gini'],
              'ccp_alpha': list(np.linspace(0, 1, 50, dtype = float))}
...
print(accuracy_score(y_ts_m1, lr.predict(X_ts_res_bor)))
print(recall_score(y_ts_m1, lr.predict(X_ts_res_bor), average='macro'))
target_names = ['Unsuspicious','Linked2Case', 'Suspicious']
print(classification_report(y_ts_m1, lr.predict(X_ts_res_bor), target_names=target_names))
...
dt_opt = DecisionTreeClassifier(max_depth=40, min_samples_split=2,criterion='entropy',ccp_alpha=0, random_state=123)
dt_opt.fit(X_tr_res_bor, y_res)
y_pred_dt = dt_opt.predict(X_ts_res_bor)

print(accuracy_score(y_ts_m1, y_pred_dt))
print(recall_score(y_ts_m1, y_pred_dt, average='macro'))
target_names = ['Unsuspicious','Linked2Case', 'Suspicious']
print(classification_report(y_ts_m1, y_pred_dt, target_names=target_names))
print(f1_score(y_ts_m1, y_pred_dt, average='macro'))
print(f1_score(y_ts_m1, y_pred_dt, average='weighted'))
print(fbeta_score(y_ts_m1, y_pred_dt, average='macro', beta=0.5))
print(fbeta_score(y_ts_m1, y_pred_dt, average='weighted', beta=0.5))

#Step 8b: New training with opt hyperparameters RF
rf_opt = RandomForestClassifier(max_depth=33, n_estimators=97,criterion='gini',ccp_alpha=0,random_state=123)
rf_opt.fit(X_tr_res_bor, y_res)
y_pred_rf = rf_opt.predict(X_ts_res_bor)

print(accuracy_score(y_ts_m1, y_pred_rf))
print(recall_score(y_ts_m1, y_pred_rf, average='macro'))
target_names = ['Unsuspicious','Linked2Case', 'Suspicious']
print(classification_report(y_ts_m1, y_pred_rf, target_names=target_names))
print(f1_score(y_ts_m1, y_pred_rf, average='macro'))
print(f1_score(y_ts_m1, y_pred_rf, average='weighted'))
print(fbeta_score(y_ts_m1, y_pred_rf, average='macro', beta=0.5))
print(fbeta_score(y_ts_m1, y_pred_rf, average='weighted', beta=0.5))


#Step 8c: New training with opt hyperparameters LR
lr_opt = LogisticRegression(solver = 'liblinear',penalty='l1',C=0.4736,random_state=123)
lr_opt.fit(X_tr_res_bor, y_res)
y_pred_lr = lr_opt.predict(X_ts_res_bor)

print(accuracy_score(y_ts_m1, y_pred_lr))
print(recall_score(y_ts_m1, y_pred_lr, average='macro'))
target_names = ['Unsuspicious','Linked2Case', 'Suspicious']
print(classification_report(y_ts_m1, y_pred_lr, target_names=target_names))
print(f1_score(y_ts_m1, y_pred_lr, average='macro'))
print(f1_score(y_ts_m1, y_pred_lr, average='weighted'))
print(fbeta_score(y_ts_m1, y_pred_lr, average='macro', beta=0.5))
print(fbeta_score(y_ts_m1, y_pred_lr, average='weighted', beta=0.5))
