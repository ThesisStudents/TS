...
## Binary Labels
cond_b2 = (df['ALERT_STATUS'] == 'Unsuspicious Alert') | \
          (df['ALERT_STATUS'] == 'Linked To Case') | \
          (df['ALERT_STATUS'] == 'Unsuspicious Case')

             
y_b2 = np.where(cond_b2, 0, 1)

## Data Split
X_tr_b2, X_ts_b2, y_tr_b2, y_ts_b2 = train_test_split(df.drop(columns = 'ALERT_STATUS'), 
                                                      y_b2,
                                                      stratify = y_b2,
                                                      test_size=0.33,
                                                      random_state=123)

X_tr_pp_b2 = pipe.fit_transform(X_tr_b2)
X_ts_pp_b2 = pipe.transform(X_ts_b2)
...
X_res, y_res = ada.fit_resample(X_tr_pp_b2, y_tr_b2) 
...
X_ts_res_bor = feat_selector.transform(X_ts_pp_b2)
...
print(accuracy_score(y_ts_b2, dt.predict(X_ts_res_bor)))
print(recall_score(y_ts_b2, dt.predict(X_ts_res_bor), average='macro'))
target_names = ['Stage 1', 'Stage 2']
print(classification_report(y_ts_b2, dt.predict(X_ts_res_bor), target_names=target_names))

#Find the optimal hyperparamaters
dt = DecisionTreeClassifier(random_state=123)
param_grid = {'max_depth': list(np.linspace(1, 87, 87, dtype = int)) + [None],
...
print(accuracy_score(y_ts_b2, rf.predict(X_ts_res_bor)))
print(recall_score(y_ts_b2, rf.predict(X_ts_res_bor), average='macro'))
target_names = ['Stage 1', 'Stage 2']
print(classification_report(y_ts_b2, rf.predict(X_ts_res_bor), target_names=target_names))

#Find the optimal hyperparameters
rf = RandomForestClassifier(random_state=123)
param_grid = {'n_estimators': list(np.linspace(1, 200, 100, dtype = int)),
              'max_depth': list(np.linspace(1, 87, 87, dtype = int)) + [None],
              'criterion': ['entropy', 'gini'],
              'min_samples_split': [2,3,4,5],
              'ccp_alpha':list(np.linspace(0, 1, 50, dtype = float))}
...
print(accuracy_score(y_ts_b2, lr.predict(X_ts_res_bor)))
print(recall_score(y_ts_b2, lr.predict(X_ts_res_bor), average='macro'))
target_names = ['Stage 1', 'Stage 2']
print(classification_report(y_ts_b2, lr.predict(X_ts_res_bor), target_names=target_names))
...
dt_opt = DecisionTreeClassifier(max_features= 'auto', max_depth= 25,min_samples_split=2, criterion= 'entropy', ccp_alpha=0, random_state=123)
dt_opt.fit(X_tr_res_bor, y_res)
y_pred_dt = dt_opt.predict(X_ts_res_bor)

print(accuracy_score(y_ts_b2, y_pred_dt))
print(recall_score(y_ts_b2, y_pred_dt, average='macro'))

target_names = ['Stage 1', 'Stage 2']
print(classification_report(y_ts_b2, y_pred_dt, target_names=target_names))
print(f1_score(y_ts_b2, y_pred_dt, average='macro'))
print(f1_score(y_ts_b2, y_pred_dt, average='weighted'))
print(fbeta_score(y_ts_b2, y_pred_dt, average='macro', beta=0.5))
print(fbeta_score(y_ts_b2, y_pred_dt, average='weighted', beta=0.5))


#Step 8b: New training with opt hyperparameters RF
rf_opt = RandomForestClassifier(n_estimators=61, max_features= 'auto', max_depth= 71, criterion= 'entropy', random_state=123)
rf_opt.fit(X_tr_res_bor, y_res)
y_pred_rf = rf_opt.predict(X_ts_res_bor)

print(accuracy_score(y_ts_b2, y_pred_rf))
print(recall_score(y_ts_b2, y_pred_rf, average='macro'))
target_names = ['Stage 1', 'Stage 2']
print(classification_report(y_ts_b2, y_pred_rf, target_names=target_names))
print(f1_score(y_ts_b2, y_pred_rf, average='macro'))
print(f1_score(y_ts_b2, y_pred_rf, average='weighted'))
print(fbeta_score(y_ts_b2, y_pred_rf, average='macro', beta=0.5))
print(fbeta_score(y_ts_b2, y_pred_rf, average='weighted', beta=0.5))


#Step 8c: New training with opt hyperparameters LR
lr_opt = LogisticRegression(solver = 'liblinear',penalty='l1',C=0.47368,random_state=123)
lr_opt.fit(X_tr_res_bor, y_res)
y_pred_lr = lr_opt.predict(X_ts_res_bor)

print(accuracy_score(y_ts_b2, y_pred_lr))
print(recall_score(y_ts_b2, y_pred_lr, average='macro'))
target_names = ['Stage 1', 'Stage 2']
print(classification_report(y_ts_b2, y_pred_lr, target_names=target_names))
print(f1_score(y_ts_b2, y_pred_lr, average='macro'))
print(f1_score(y_ts_b2, y_pred_lr, average='weighted'))
print(fbeta_score(y_ts_b2, y_pred_lr, average='macro', beta=0.5))
print(fbeta_score(y_ts_b2, y_pred_lr, average='weighted', beta=0.5))